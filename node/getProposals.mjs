...
const date = new Date(2025, 8, 22); // July is month 6 (0-based)
...
const allRealms = [];
const deployments_output_dir = 'output_deployments_2025_9_22';
...
for (let i = allRealms.length - 1 ; i >= 0; i--) {
    ...
    // ✅ Skip if votingProposalCount == 0
    if (realm.account && realm.account.votingProposalCount === 0) {
        console.log('skipping (no proposals)', programId, realmPubKeyString);
        continue;
    }
